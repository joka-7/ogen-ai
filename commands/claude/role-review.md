...
```
Then append `.ai-reviews/` to `<workdir>/.git/info/exclude` if it isn't already there. This keeps reports out of `git status` without touching the project's committed `.gitignore`. Confirm afterwards that `git status --short` in the target is clean.
...
The reviewers have no write access by design — they return their report as their final message. Write each verbatim to `<workdir>/.ai-reviews/<role>.md`. Do not edit, summarize, or reformat them on the way to disk; they are the evidence record the next step and the human both depend on.

**6. Aggregate.**
Launch `planner`. It reads the report files from disk and returns a prioritized backlog; write that to `<workdir>/.ai-reviews/BACKLOG.md`.
